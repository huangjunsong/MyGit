...
- (NSMutableArray *)extractQuery:(NSMutableString *)query where:(id)where
{
    ...
    {
        NSDictionary *dict = where;
        if ([dict count] > 0) {
            values = [NSMutableArray arrayWithCapacity:[dict count]];
...
{
    checkClassIsInvalid(modelClass);
    NSString* tableName = [modelClass getTableName];//非常聪明的运用了泛型概念
...
-(void)sqlString:(NSMutableString*)sql AddOder:(NSString*)orderby offset:(int)offset count:(int)count
{
    ...
    if (count>0) // add  by daoyou
    {
          [sql appendFormat:@" limit %d offset %d",count,offset];//limit后面跟的是 多少 条数据，offset后面是从第  几 条开始读取
    }
